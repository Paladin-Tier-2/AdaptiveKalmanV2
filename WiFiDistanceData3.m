...
if isempty(baseDir)
    error('Base directory could not be determined.');
end

% Specify the filename
fileName = 'measurements.csv'; % Name of the CSV file
fullFilePath = fullfile(baseDir, fileName); % Constructs full file path

% Check if the file exists
if ~isfile(fullFilePath)
    error('The file "%s" does not exist in the directory "%s".', fileName, baseDir);
end
